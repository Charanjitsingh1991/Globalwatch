...
import { fetchCyberThreats } from '@/services/cyberService'

const CACHE_KEY = 'globalwatch:cyber:all'
const TTL = 600
...
export async function GET() {
  ...
  try {
    ...
    const result = {
      ...
      source: 'alienvault-otx', count: data.length,
    }
    ...
    logger.error('Cyber fetch failed', error)
    if (redis) {
